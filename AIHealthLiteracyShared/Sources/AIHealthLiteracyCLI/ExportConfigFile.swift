...
struct ExportConfigFile: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: #"""
            Note: in order to create the sample plist file that is commited to the repo, use the following command:
                swift run AIHealthLiteracyCLI export-config -f '<emulator>' --allow-empty-api-keys ../AIHealthLiteracy/Supporting\ Files/UserStudyConfig.plist
            """#
    )
    ...
    private func studyValue<V>(
        ...
        if let value = _studyValue(for: studyId, in: values) {
            ...
            throw NSError(domain: "edu.stanford.aihealthliteracy.CLI", code: 0, userInfo: [
