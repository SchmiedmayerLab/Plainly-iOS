...
struct StudyHomeView: View {
    ...
    @Environment(AIHealthLiteracyStandard.self) private var standard
    ...
    private var studyTitle: some View {
        VStack(spacing: 8) {
            Text(fhirInterpretationModule.currentStudy?.study.title ?? "AIHealthLiteracy")
                .font(.title)
                .fontWeight(.bold)
    ...
    @ViewBuilder private var recordsStartDateView: some View {
        if let oldestHealthRecordTimestamp {
            Button {
                ...
                Text("HEALTH_RECORDS_SINCE: \(oldestHealthRecordTimestamp, format: .aiHealthLiteracyOldestHealthSample)")
                    .font(.caption)
                    .foregroundColor(.secondary)
                    .fontWeight(.medium)
                    .underline()
