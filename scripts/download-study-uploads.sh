...
# Usage: download-study-uploads.sh --project <firebase project> --study <id>|all [--destination <dir>]
set -euo pipefail
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *) echo "usage: $(basename "$0") --project <firebase project> --study <id>|all [--destination <dir>]" >&2; exit 1;;
  esac
done
if [ -z "$project" ] || [ -z "$study" ]; then
  echo "usage: $(basename "$0") --project <firebase project> --study <id>|all [--destination <dir>]" >&2
  exit 1
fi
